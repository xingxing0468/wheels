...
class ZServicePackageHelper():
    ...
    def Package(service_name, method_index, serialized_data):
        package_size = len(ZServicePackageHelper._signature_) + \
            len(service_name) + len(serialized_data) + \
            4 + 4 + 4  # len(service name) + len(int) + method_index(int)
    ...
    def UnPackage(package_data):
        ...
        # signature
        signature = str(package_data[offset: (
            offset + len(ZServicePackageHelper._signature_))].decode('utf-8'))
        if signature != ZServicePackageHelper._signature_:
            print("Error: Invalid signature, expected: [{}], actual: [{}]".format(
        ...
        if package_size != len(package_data):
            print("Error: Invalid package size, expected: [{}], actual: [{}]".format(
