...
constexpr std::string_view Magic = "Zed";

constexpr std::uint32_t GetPackageHeaderSize() {
  // Magic length + 4 byte package size + 4 byte service name size
  return Magic.size() + sizeof(uint32_t) + sizeof(uint32_t);
};

std::vector<uint8_t> Pack(const std::string& service_name,
  ...
  uint32_t package_size = GetPackageHeaderSize() + service_name_size +
                          sizeof(uint32_t) +  // method index
                          serialized_data.size();

  ret.insert(ret.end(), reinterpret_cast<uint8_t*>(&package_size),
             reinterpret_cast<uint8_t*>(&package_size) + sizeof(uint32_t));

  ret.insert(ret.end(), reinterpret_cast<uint8_t*>(&service_name_size),
             reinterpret_cast<uint8_t*>(&service_name_size) + sizeof(uint32_t));

  ret.insert(ret.end(), service_name.c_str(),
             service_name.c_str() + service_name.size());

  ret.insert(
      ret.end(), reinterpret_cast<const uint8_t*>(&method_index),
      reinterpret_cast<const uint8_t*>(&method_index) + sizeof(uint32_t));

  ret.insert(ret.end(), serialized_data.data(),
             serialized_data.data() + serialized_data.size());
  return ret;
};

std::vector<uint8_t> Unpack(const std::vector<uint8_t> package_data,
                            std::string& service_name, uint32_t& method_index) {
  std::vector<uint8_t> ret{};
  ...
  int offset = 0;
  if (memcmp(package_data.data(), Magic.data(),
             Magic.size())) {  // Signature mismatch
    return ret;
  }
  offset += Magic.size();
  ...
  service_name.clear();
  service_name.assign(package_data.data() + offset,
                      package_data.data() + offset + service_name_size);
